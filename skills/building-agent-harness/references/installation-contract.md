...
Migration is a separate exact migration change package, such as `state-migrate-v1-to-v2`, that requires its own approval. Before proposing it, prove every artifact-to-change-ID association without guesswork and, for every existing file, prove the recorded pre-install bytes resolve to an exact reconstructible Git blob whose bytes match `preInstallHash`. An original approved install record may prove a mapping; a single installed change ID is also unambiguous for the v1 artifacts. Never infer among multiple plausible IDs or invent a Git object ID from current content.

Build and validate the complete schema-2 replacement before one atomic state-file write. If any association, path ownership, hash, or pre-install Git blob is ambiguous or unavailable, leave the v1 state byte-for-byte untouched and report that migration needs human resolution.
